#ifndef QST_OBSERVERDENSITYMATRIX_HPP
#define QST_OBSERVERDENSITYMATRIX_HPP
...
template<class NNState> class ObserverDensityMatrix{

    NNState &RHO_;

    int N_;
    int npar_;
    Eigen::VectorXcd target_rho_;
    std::vector<Eigen::VectorXcd> rotated_rho_;
...
public:
    ...
    ObserverDensityMatrix(NNState &RHO,std::string &basis):RHO_(RHO){ 
    ...
    void ExactPartitionFunction() {
        ...
        for(int i=0;i<basis_states_.rows();i++){
            Z_ += RHO_.rho(basis_states_.row(i)).real();
        }
    }
